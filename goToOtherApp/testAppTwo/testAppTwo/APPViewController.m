...
- (void)viewDidLoad {
    ...
    self.testLabel1.text = @"点击上方紫色按钮，可把textField输入的字符串，传递到testAppOne里面。前提需要把testAppOne、testAppTwo都运行在模拟机或真机上。";
    ...
    [btnApp setTitle:@"testAppOne" forState:UIControlStateNormal];
...
- (void)testApp
{
    ...
    //执行openURL 跳转APP
    NSString *str = [NSString stringWithFormat:@"testAppOne://com.lakala.MeiChi?name=%@",self.textField.text];
...
- (void)testAppstore
{
    NSURL *myURl_APP_B = [NSURL URLWithString:@"weixin:"];
